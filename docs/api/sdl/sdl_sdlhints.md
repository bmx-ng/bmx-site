...
This will reset a hint to the value of the environment variable, or [Null](../../brl/brl.blitz/#null) if
the environment isn't set. Callbacks will be called normally with this change.


#### Returns
[True](../../brl/brl.blitz/#true) if the hint was set, [False](../../brl/brl.blitz/#false) otherwise
...
variable, or [Null](../../brl/brl.blitz/#null) if the environment isn't set. Callbacks will be called normally with this change.
...
The string value of a hint or [Null](../../brl/brl.blitz/#null) if the hint isn't set.
...
variable that takes precedence. You can use [SDLSetHintWithPriority](../../sdl/sdl.sdl/#function-sdlsethintwithpriorityintnamestring-valuestring-priorityesdlhintpriority)() to
...
```
This hint must be set together with the hint [SDL_HINT_ANDROID_APK_EXPANSION_PATCH_FILE_VERSION](../../sdl/sdl.sdlhints/#const-sdlhintandroidapkexpansionpatchfileversionstring-sdlandroidapkexpansionpatchfileversion).
...
```
This hint must be set together with the hint [SDL_HINT_ANDROID_APK_EXPANSION_MAIN_FILE_VERSION](../../sdl/sdl.sdlhints/#const-sdlhintandroidapkexpansionmainfileversionstring-sdlandroidapkexpansionmainfileversion).
...
This is necessary for the rightmouse button to work on some Android devices, or to be able to trap the back
...
Some audio backends (such as Pipewire) allow you to describe the role of your audio stream.
...
If available, SDL can use libsamplerate ( http://www.mega-nerd.com/SRC/ ) to handle audio resampling. There are
...
If set, this hint will override the expected results for SDL_GetDisplayUsableBounds( ) for display index 0.
...
A hint that specifies how 3D acceleration is used with [GetSurface](../../sdl/sdl.sdlvideo/tsdlwindow/#method-getsurfacetsdlsurface)().
...
SDL can try to accelerate the screen surface returned by TSDLWindow [GetSurface](../../sdl/sdl.sdlvideo/tsdlwindow/#method-getsurfacetsdlsurface)() by using streaming textures with a 3D rendering engine.
...
```
The hint can also take the form of <b>file</b>, in which case the named file will be loaded and interpreted as the value of the variable.
...
0xAAAA/0xBBBB,0xCCCC/0xDDDD
```
The variable can also take the form of <b>file</b>, in which case the named file will be loaded and interpreted as the value of the variable.
...
As of SDL 2.0.4, [SDLEnableScreenSaver](../../sdl/sdl.sdlvideo/#function-sdlenablescreensaver)() and [SDLDisableScreenSaver](../../sdl/sdl.sdlvideo/#function-sdldisablescreensaver)() accomplish the same thing on iOS. They should be preferred over this hint.
...
value will also control the state of extended reports on PS5 controllers when the [SDL_HINT_JOYSTICK_HIDAPI_PS5_RUMBLE](../../sdl/sdl.sdlhints/#const-sdlhintjoystickhidapips5rumblestring-sdljoystickhidapips5rumble) hint is not explicitly set.
...
1   HIDAPI driver is used
```
The default is the value of [SDL_HINT_JOYSTICK_HIDAPI](../../sdl/sdl.sdlhints/#const-sdlhintjoystickhidapistring-sdljoystickhidapi).
...
defaults to the value of [SDL_HINT_JOYSTICK_HIDAPI_PS4_RUMBLE](../../sdl/sdl.sdlhints/#const-sdlhintjoystickhidapips4rumblestring-sdljoystickhidapips4rumble).
...
1   HIDAPI driver is used
```
The default is 0 on Windows, otherwise the value of [SDL_HINT_JOYSTICK_HIDAPI](../../sdl/sdl.sdlhints/#const-sdlhintjoystickhidapistring-sdljoystickhidapi).
...
The DRM subsystem has a concept of a "DRM master" which is a DRM client that has the ability to set planes, set cursor,
...
On some platforms, currently Windows and X11, OpenGL drivers may support creating contexts with
...
This variable can be set to the following values: 0 or nearest Nearest pixel sampling (default) 1 or linear
...
On some platforms, like Linux, a realtime priority thread may be subject to restrictions that require special handling by
...
By default SDLSetThreadPriority will make appropriate system changes in order to apply a thread priority.
...
The higher resolution the timer, the more frequently the CPU services timer interrupts, and the more precise delays are,
...
Defaults to [False](../../brl/brl.blitz/#false).
...
A hint that is the address of another SDL Window* (as a hex string formatted with <i>p</i>).

If this hint is set before CreateWindowFrom( ) and the SDL Window* it is set to has SDL_WINDOW_OPENGL set (and running on WGL only, currently),
...
This hint can be set to the address (as a string <i>p</i>) of the SDL_Window* that new windows created with SDL_CreateWindowFrom( ... )
...
SDL has EGL and OpenGL ES2 support on Windows via the ANGLE project. It can use two different sets of binaries, those
...
The fact chunk stores information about the number of samples of a WAVE file. The Standards Update
...
A WAVE file is considered truncated if any of the chunks are incomplete or the data chunk size
...
The 0x406D1388 Exception is a trick used to inform Visual Studio of a thread's name, but it tends to cause problems
...
On Windows 7 and newer, Slim Reader/Writer Locks are available. They offer better performance, allocate no kernel
...
Direct3D 9Ex contains changes to state management that can eliminate device loss errors during scenarios
...
Windows Phone devices typically feature a Back button. When pressed, the OS will emit back-button-press events,
...
Network-enabled WinRT apps must include a privacy policy. On Windows 8, 8.1, and RT, Microsoft mandates that this policy be
...
hint, [SDL_HINT_WINRT_PRIVACY_POLICY_LABEL](../../sdl/sdl.sdlhints/#const-sdlhintwinrtprivacypolicylabelstring-sdlwinrtprivacypolicylabel).

Please note that a privacy policy's contents are not set via this hint. A separate hint, [SDL_HINT_WINRT_PRIVACY_POLICY_URL](../../sdl/sdl.sdlhints/#const-sdlhintwinrtprivacypolicyurlstring-sdlwinrtprivacypolicyurl),
...
For additional information on linking to a privacy policy, see the documentation for [SDL_HINT_WINRT_PRIVACY_POLICY_URL](../../sdl/sdl.sdlhints/#const-sdlhintwinrtprivacypolicyurlstring-sdlwinrtprivacypolicyurl).
...
All network-enabled WinRT apps must make a privacy policy available to its users. On Windows 8, 8.1, and RT, Microsoft mandates
...
To setup a URL to an app's privacy policy, set [SDL_HINT_WINRT_PRIVACY_POLICY_URL](../../sdl/sdl.sdlhints/#const-sdlhintwinrtprivacypolicyurlstring-sdlwinrtprivacypolicyurl) before calling any SDL_Init( ... ) functions. The
...
The label text of an app's "Privacy Policy" link may be customized via another hint, [SDL_HINT_WINRT_PRIVACY_POLICY_LABEL](../../sdl/sdl.sdlhints/#const-sdlhintwinrtprivacypolicylabelstring-sdlwinrtprivacypolicylabel).
...
[SDL_HINT_WINRT_PRIVACY_POLICY_URL](../../sdl/sdl.sdlhints/#const-sdlhintwinrtprivacypolicyurlstring-sdlwinrtprivacypolicyurl) will not get used on that platform. Network-enabled phone apps should display their privacy policy
